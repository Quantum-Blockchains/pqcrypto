...
/// Generate a ml-dsa-65 keypair from seed
pub fn keypair_from_seed(seed: &[u8; 32]) -> (PublicKey, SecretKey) {
    #[cfg(all(enable_x86_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_keypair_from_seed!(PQCLEAN_MLDSA65_AVX2_crypto_sign_keypair_from_seed, seed);
        }
    }
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
    ...
    gen_keypair_from_seed!(PQCLEAN_MLDSA65_CLEAN_crypto_sign_keypair_from_seed, seed)
}
